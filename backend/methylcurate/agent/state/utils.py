...
def make_geo_ingestion_state(run_id: str, params: dict[str, Any]) -> GeoIngestionSubgraphState:
    ...
    for accession_code in config.accessions:
        datasets[accession_code] = GeoDatasetState.model_validate(
            {"accession": accession_code, "output_dir": os.path.join(config.output_root, accession_code)}
        )
...
def make_quality_control_state(run_id: str, params: dict[str, Any]) -> QualityControlSubgraphState:
    ...
    config_path = os.getenv("QC_CONFIG_PATH")
    if config_path:
        qc_config = QcConfig.from_yaml(config_path)
        if qc_config is not None:
            if qc_config.dnam_cutoff is not None:
                state_kwargs["dnam_qc_input"] = DNAmQCInput(dnam_cutoff=qc_config.dnam_cutoff)
            if qc_config.sample_level_missing_cutoff is not None:
                state_kwargs["sample_level_qc_input"] = SampleLevelQCInput(missing_cutoff=qc_config.sample_level_missing_cutoff)
            if qc_config.cpg_level_missing_cutoff is not None:
                state_kwargs["cpg_level_qc_input"] = CpGLevelQCInput(missing_cutoff=qc_config.cpg_level_missing_cutoff)
            if qc_config.correlation_cutoff is not None:
                state_kwargs["interarray_correlation_qc_input"] = InterarrayCorrelationQCInput(correlation_cutoff=qc_config.correlation_cutoff)

    return QualityControlSubgraphState(run_id=run_id, config=config, datasets=datasets, **state_kwargs)
...
def make_help_state(run_id: str, params: dict[str, Any]) -> HelpSubgraphState:
    config = HelpIngestionConfig.model_validate(params)
    return HelpSubgraphState(run_id=run_id, config=config)
...
def make_subgraph_state(
    ...
    if subgraph == "geo_retrieval":
        ...
        return make_help_state(run_id, params)
    else:
...
def make_full_subgraph_state(
    ...
    if subgraph == "geo_retrieval":
        ...
        return HelpSubgraphState.model_validate(params)
    else:
        raise ValueError(f"Unknown subgraph: {subgraph}")
...
def make_main_state(
    *,
    ...
    ) = generate_router_clarification_examples(output_root=output_dir)
    ...
    return MainState(
        ...
        messages=[
            ...
            SystemMessage(
                content=generate_router_system_prompt(
                    root_dir=output_dir,
...
PARAM_SCHEMAS["help"] = HelpIngestionConfig
