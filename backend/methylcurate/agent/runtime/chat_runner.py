...
from ...contracts.common import StepStatus
...
class StreamingRunner:
    ...
    async def _load_or_create_sub_state(self, subgraph: Any, subgraph_name: str, run_id: str, thread_id: str, params: dict[str, Any]):
        ...
        dataset_state_class = {
            "geo_retrieval": GeoDatasetState,
            "harmonization": HarmonizationDatasetState,
            "quality_control": DatasetQualityControlState,
            "benchmarking": BenchmarkingDatasetState,
        }.get(subgraph_name)

        if dataset_state_class is not None:
            for accession in sub_state.config.accessions:
                if accession not in sub_state.datasets:
                    sub_state.datasets[accession] = dataset_state_class.model_validate(
                        {
                            "accession": accession,
                            "output_dir": os.path.join(sub_state.config.output_root, accession),
                        }
                    )
        return sub_state
    ...
    def _populate_main_datasets(self, artifacts: list[Any]) -> dict[str, Any]:
        ...
        for a in all_geo_completed:
            ...
            step_statuses[accession_code]["geo_retrieval"] = StepStatus(status="completed")
        all_qc_completed = [a for a in artifacts if a.kind == "postqc_methylation_data"]
        for a in all_qc_completed:
            accession_code = a.accession_code
            step_statuses[accession_code]["quality_control"] = StepStatus(status="completed")

        all_benchmarking_completed = [a for a in artifacts if a.kind == "benchmark_summary"]
        for a in all_benchmarking_completed:
            accession_code = a.accession_code
            step_statuses[accession_code]["benchmarking"] = StepStatus(status="completed")
    ...
    async def stream_main_then_subgraph(
        ...
        *,
        ...
        cp_artifacts = list((checkpointed or {}).get("artifacts", []) or [])
        cp_datasets = dict((checkpointed or {}).get("datasets", {}) or {})
        rebuilt_from_artifacts = self._populate_main_datasets(cp_artifacts)
        merged_datasets = dict(cp_datasets)
        for accession, steps in rebuilt_from_artifacts.items():
            merged_datasets.setdefault(accession, {}).update(steps)
        if isinstance(main_state, dict):
            main_state["artifacts"] = cp_artifacts
            main_state["datasets"] = merged_datasets
        ...
        await self._persist_main_dataset_statuses(main_thread, main_datasets)
        ...
        try:
            ...
            logger.exception("Subgraph %s stream crashed", subgraph_name)
            await queue.put(StreamEvent("final", {"message": f"Subgraph {subgraph_name} crashed. Check server logs for details."}))
            raise
        ...
        try:
            ...
            if self.deps.provenance is not None:
                ...
                for a in sub_artifacts:
                    if isinstance(a, dict):
                        manifest.append({"kind": a.get("kind"), "path": a.get("path"), "sha256": a.get("sha256")})
                    else:
            ...
            updated_datasets = self._populate_main_datasets(merged)
            await self._persist_main_dataset_statuses(main_thread, updated_datasets)
            ...
            logger.exception("Artifact merge/persist after subgraph %s failed", subgraph_name)
            await queue.put(StreamEvent("final", {"message": f"Error merging results from {subgraph_name}. Check server logs for details."}))
            raise
