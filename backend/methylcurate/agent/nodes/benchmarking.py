...
from ...utils.helper import (
    get_accession_codes, consolidate_artifacts, benchmarking_progress, compute_sha256,
    load_metadata_aligned_methylation_data, PROJECT_ROOT)
...
def _get_harmonized_full_data_if_available(
        accession_code: str, metadata_df: pd.DataFrame, methylation_df: pd.DataFrame, artifacts: List[ArtifactRef]) -> pd.DataFrame:
    """
...
def benchmarking_node(state: BenchmarkingSubgraphState, *, config: RunnableConfig) -> Command:
    ...
    if internal_clock_list:
        internal_clock_predictions = make_internal_clock_predictions(full_data, internal_clock_list, metadata_cols=metadata_columns + methylation_metadata_col, imputer_strategy='knn')
        adata.obs = adata.obs.merge(internal_clock_predictions, on=metadata_columns + methylation_metadata_col, how='left')
...
def summarize_benchmarking_results(state: BenchmarkingSubgraphState, *, config: RunnableConfig) -> Command:
    ...
    for accession_code in accession_codes:
        computation_artifact = next((a for a in state.config.artifacts if a.kind == "benchmark_summary" and a.accession_code == accession_code), None)
    ...
    for _, row in task_subset.dropna(subset=["AA1_score", "AA1_emp_score"]).iterrows():
        aa1_payload["rows"].append({
            ...
            "dx": row["Disease"],
            "aa1": row["AA1_score"],
            "aa1_shuffled": row["AA1_emp_score"]
        })
    ...
    for _, row in task_subset.dropna(subset=["AA2_score", "AA2_emp_score"]).iterrows():
        aa2_payload["rows"].append({
            ...
            "dx": row["Disease"],
            "aa2": row["AA2_score"],
            "aa2_shuffled": row["AA2_emp_score"]
        })

    # predictive_message
    predictive_performance_payload["message"] = f"We benchmarked {len(state.config.clock_list)} clocks on {len(accession_codes)} datasets from GEO. Here is a summary of their performance:"
    ...
    aa1_payload["message"] = f"Here is a summary of AA1 performance across the {len(state.config.clock_list)} clocks and {len(accession_codes)} datasets we benchmarked:"
    ...
    aa2_payload["message"] = f"Here is a summary of AA2 performance across the {len(state.config.clock_list)} clocks and {len(accession_codes)} datasets we benchmarked:"
