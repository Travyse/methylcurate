...
def _save_artifact_pair(return_dict, accession_code, guessed, harmonized, kind_prefix, output_dir, config=None):
    ...
    Args:
        ...
        config: Optional RunnableConfig for provenance logging.
    ...
    guessed_sha = compute_sha256(guessed_path, is_path=True)
    harmonized_sha = compute_sha256(harmonized_path, is_path=True)
    artifacts = [
    ...
    if config is not None:
        deps: Deps = config["configurable"].get("deps")  # type: ignore[assignment]
        if deps is not None and deps.provenance is not None:
            prov = deps.get_provenance(config["configurable"]["thread_id"])
            if prov is not None:
                step = return_dict.get("_current_step")
                for a in artifacts:
                    prov.emit_artifact_written(
                        artifact_kind=a.kind,
                        artifact_path=a.path,
                        artifact_sha256=a.sha256,
                        artifact_bytes=a.bytes,
                        accession_code=accession_code,
                        step=step,
                    )
    return artifacts
...
async def disease_harmonization_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    if any(x is None for x in [metadata_cache, sample_metadata, extraction_protocol]):
        ...
        completed_state = state.model_copy(deep=True)
        completed_state.datasets[accession_code].steps["harmonize_sex_labels"].status = "completed"
        return_dict["messages"] = [update_harmonization_progress_tracker(completed_state)]
        return_dict["main_messages"] = [update_harmonization_progress_tracker(completed_state)]
    ...
    return_dict["_current_step"] = "map_disease_labels_to_ontology"
    _save_artifact_pair(
        return_dict,
        accession_code,
        guessed_disease_labels,
        harmonized_disease_labels,
        "disease",
...
async def higher_level_disease_mapping_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    running_accession_codes = sorted(
        [
            accession_code
            for accession_code in accession_codes
            if state.datasets[accession_code].steps["group_disease_labels"].status in {"running", "not_started"}
        ]
    )
    ...
    for accession_code in running_accession_codes:
        ...
        disease_sha = compute_sha256(harmonization_result_path, is_path=True)
        disease_bytes = os.path.getsize(harmonization_result_path)
        artifacts.append(
            ArtifactRef.model_validate(
                {
                    "accession_code": accession_code,
        ...
        deps: Deps = config["configurable"].get("deps")  # type: ignore[assignment]  # ty: ignore
        if deps is not None and deps.provenance is not None:
            prov = deps.get_provenance(config["configurable"]["thread_id"])
            if prov is not None:
                prov.emit_artifact_written(
                    artifact_kind="disease_harmonization_mapping",
                    artifact_path=harmonization_result_path,
                    artifact_sha256=disease_sha,
                    artifact_bytes=disease_bytes,
                    accession_code=accession_code,
                    step="group_disease_labels",
                )
...
async def tissue_harmonization_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    return_dict["_current_step"] = "map_tissue_labels_to_ontology"
    _save_artifact_pair(
        return_dict,
        accession_code,
        guessed_tissue_labels,
        harmonized_tissue_labels,
        "tissue",
        state.datasets[accession_code].output_dir,
        config=config,
    )
...
async def higher_level_tissue_mapping_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    running_accession_codes = sorted(
        [
            accession_code
            for accession_code in accession_codes
            if state.datasets[accession_code].steps["group_tissue_labels"].status in {"running", "not_started"}
        ]
    )
    ...
    for accession_code in running_accession_codes:
        ...
        proper_mapping = construct_raw_to_harmonized_label_mapping(tissue_label_guessing, tissue_label_mapping)
        for mapping in proper_mapping.mappings:
            harmonized_group_label = next(
                (
                    group_mapping.target_label  # type: ignore
        ...
        harmonization_result_df.to_csv(harmonization_result_path, index=True)
        tissue_sha = compute_sha256(harmonization_result_path, is_path=True)
        tissue_bytes = os.path.getsize(harmonization_result_path)
        artifacts.append(
            ArtifactRef.model_validate(
                {
                    "accession_code": accession_code,
        ...
        if deps is not None and deps.provenance is not None:
            ...
            if prov is not None:
                prov.emit_artifact_written(
                    artifact_kind="tissue_harmonization_mapping",
                    artifact_path=harmonization_result_path,
                    artifact_sha256=tissue_sha,
                    artifact_bytes=tissue_bytes,
                    accession_code=accession_code,
                    step="group_tissue_labels",
                )
...
async def cell_type_harmonization_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    return_dict["_current_step"] = "map_cell_type_labels_to_ontology"
    _save_artifact_pair(
        return_dict,
        accession_code,
        guessed_cell_type_labels,
        harmonized_cell_type_labels,
        "cell_type",
        state.datasets[accession_code].output_dir,
        config=config,
    )
...
async def sex_harmonization_node(state: HarmonizationSubgraphState, config: RunnableConfig):
    ...
    running_accession_codes = sorted(
        [
            accession_code
            for accession_code in accession_codes
            if state.datasets[accession_code].steps["harmonize_sex_labels"].status in {"running", "not_started"}
        ]
    )
    accession_code = running_accession_codes.pop(0)
    harmonization_dir = os.path.join(state.datasets[accession_code].output_dir, "harmonization")
    ...
    sex_sha = compute_sha256(harmonized_sex_labels_path, is_path=True)
    sex_bytes = os.path.getsize(harmonized_sex_labels_path)
    artifacts.append(
        ArtifactRef.model_validate(
            {
                "accession_code": accession_code,
    ...
    if deps is not None and deps.provenance is not None:
        ...
        if prov is not None:
            prov.emit_artifact_written(
                artifact_kind="sex_label_harmonization",
                artifact_path=harmonized_sex_labels_path,
                artifact_sha256=sex_sha,
                artifact_bytes=sex_bytes,
                accession_code=accession_code,
                step="harmonize_sex_labels",
            )
    return_dict["config"]["artifacts"] = consolidate_artifacts([ArtifactRef(**a) for a in return_dict["config"]["artifacts"]], artifacts)
