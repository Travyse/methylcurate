...
from ....utils.memory import collect as mem_collect
from ....utils.memory import init_tracker
...
def _cancel_downstream_steps(return_dict, accession_code):
    ...
    for step_name in ("extract_metadata_schema", "refine_metadata_schema", "extract_data", "supplementary_file_check"):
...
def start_geo_subgraph(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    init_tracker(os.path.join(os.path.dirname(state.config.output_root), "logs", "memory_tracker.log"))
    mem_snap("subgraph:start")
    return_dict = {}
...
def geo_download_node(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    ...
    """
    mem_snap("downloads:geo_download_node:entry")
    return_dict = {
    ...
    running_accession_codes = [
        ...
        if (
            (not state.datasets.get(x, False))
            and (state.datasets[x].steps["download_soft"].status != "completed")
            and (state.datasets[x].steps["check_valid_dataset"].status != "completed")
        )
    ...
    download_inputs = [
        {"accession": accession_code, "output_root": os.path.join(state.config.output_root, accession_code)}
        for accession_code in running_accession_codes
    ]
    batch_download_input = GEODownloadBatchInput.model_validate({"geo_downloads": download_inputs})

    artifacts, supplementary_files, batch_result = download_geo_datasets(state.config, batch_download_input)
    mem_snap("downloads:after_download_geo_datasets")

    deps_init = config["configurable"].get("deps")
    provenance = (
        deps_init.get_provenance(config["configurable"]["thread_id"])
    ...
    for accession_code in running_accession_codes:
        ...
        if not has_succeeded and provenance is not None:
            provenance.emit_error_raised(
                error_type="DownloadFailed",
                error_message=dataset_result.error if dataset_result else "Unknown download error",
                error_code="GEO_DOWNLOAD_FAILURE",
    ...
    return_dict["main_messages"] = [update_progress_tracker(state)]
    return_dict["messages"] = [update_progress_tracker(state)]
    mem_collect("downloads:geo_download_node:exit_collect")
    return Command(update=return_dict)
...
def _check_downloads_succeeded(return_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    if download_result["status"] == "failed":
        ...
        return_dict["datasets"][accession_code]["steps"]["extract_metadata_schema"] = set_step_status(
            status="canceled", step=return_dict["datasets"][accession_code]["steps"]["extract_metadata_schema"]
        )
        return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"] = set_step_status(
            status="canceled", step=return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"]
        )
        return_dict["datasets"][accession_code]["steps"]["extract_data"] = set_step_status(
            status="canceled", step=return_dict["datasets"][accession_code]["steps"]["extract_data"]
        )
        return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"] = set_step_status(
            status="canceled", step=return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"]
        )
        return_dict["datasets"][accession_code]["errors"].append(
            download_result.error
        )  # TODO: Check to see if dataset doesn't exist vs other kinda error

    return return_dict


def check_downloads_succeeded(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("downloads:check_downloads_succeeded:entry")
    accession_codes = get_accession_codes(state)
...
def check_platforms_used(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("downloads:check_platforms_used:entry")
    accession_codes = get_accession_codes(state)
    unstarted_accession_codes = sorted([accession_code for accession_code in accession_codes if not state.datasets.get(accession_code, False)])
    running_accession_codes = sorted(
        [
...
def _check_if_data_present(artifacts: list[Any], return_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    feather_size = os.path.getsize(methylation_data_path.path)  # type: ignore
    if feather_size < 2048:
            ...
            selection = interrupt(
                ToolMessage(
                    ...
                    artifact={
                        "id": hash,
                        "options": [
                            {"id": os.path.basename(supplementary_file), "label": os.path.basename(supplementary_file)}
                            for supplementary_file in supplementary_files
                        ],
                        "actions": [
                            {"id": "skip", "label": "Skip this dataset"},
                            {"id": "confirm", "label": "Confirm selection"},
                        ],
                    },
                    additional_kwargs={
        ...
        return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"] = set_step_status(
            status="not_started", step=return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"]
        )
        return_dict["datasets"][accession_code]["steps"]["extract_data"] = set_step_status(
            status="not_started", step=return_dict["datasets"][accession_code]["steps"]["extract_data"]
        )
        return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"] = set_step_status(
            status="not_started", step=return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"]
        )
        return_dict["datasets"][accession_code]["is_valid_dataset"] = True
    return return_dict
...
async def check_data_presence(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("downloads:check_data_presence:entry")
    accession_codes = get_accession_codes(state)
    download_completion = check_step_completion("download_soft", state.datasets, accession_codes)
    valid_check_completion = check_step_completion("check_valid_dataset", state.datasets, accession_codes)
    completed = download_completion and valid_check_completion
