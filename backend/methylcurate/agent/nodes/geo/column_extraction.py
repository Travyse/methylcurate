...
from ....utils.helper import get_accession_codes, compute_sha256, set_step_status, update_progress_tracker, check_step_completion, consolidate_artifacts
...
def _randomly_sample_from_dataset(
        metadata_dict: Dict[str, Any], return_dict: Dict[str, Any], 
        accession_code: str, artifact: ArtifactRef, num_samples: int = 10) -> List[Any]:
    extraction_examples = {
...
async def extract_metadata_schema(state: GeoIngestionSubgraphState, *, config: RunnableConfig) -> Dict[str, Any]:
    ...
    running_accession_codes = sorted([accession_code for accession_code in accession_codes if state.datasets[accession_code].steps["extract_metadata_schema"].status == "running"])
    ...
    extraction_result = await _extract_all_columns(
        messages, config, GSESpecificMetadataExtractionResult)
    ...
    return_dict["datasets"][accession_code]["steps"]["extract_metadata_schema"] = set_step_status(status="completed", step=return_dict["datasets"][accession_code]["steps"]["extract_metadata_schema"])
    return_dict["datasets"][accession_code]["steps"]["extract_data"] = set_step_status(status="running", step=return_dict["datasets"][accession_code]["steps"]["extract_data"])
...
async def check_column_extraction_rule_formatting(state: GeoIngestionSubgraphState, *, config: RunnableConfig) -> Dict[str, Any]:
    ...
    running_accession_codes = sorted([accession_code for accession_code in accession_codes if state.datasets[accession_code].steps["refine_metadata_schema"].status == "running"])
    ...
    metadata_cache_artifact = next((artifact for artifact in state.config.artifacts if (artifact.kind == "metadata_cache") and (artifact.accession_code == accession_code)), None)
    with open(metadata_cache_artifact.path, "r", encoding="utf-8") as f:
        metadata_dict = json.load(f)
    metadata_artifact = next((artifact for artifact in state.config.artifacts if (artifact.kind == "dataset_metadata") and (artifact.accession_code == accession_code)), None)
    ...
    return_dict = generate_summary_data(
        metadata, accession_code, [dataset_state.platform_metadata.platform_id], [dataset_state.platform_metadata.title],
        dataset_state.refinement_history.example_errors, return_dict)
...
async def check_column_extraction_rule_accuracy(state: GeoIngestionSubgraphState, *, config: RunnableConfig) -> Dict[str, Any]:
    ...
    resolutions = _get_extraction_resolutions(extraction_result)
    failed_parsing_info = return_dict["datasets"][accession_code]["refinement_history"]["example_errors"][-1] if len(return_dict["datasets"][accession_code]["refinement_history"]["example_errors"]) > 0 else {}
    flagged_concepts = [c for c in get_args(Concept) if (extraction_result.model_dump()[c]["status"] == "resolved") and (c in parse_rates) and (parse_rates[c] > 0.0 and parse_rates[c] < 1.0) and len(failed_parsing_info[c]) > 0]
    ...
    if len(flagged_concepts) > 0:
        ...
        new_resolutions = await _extract_column_for_concept_poor_parsing(
            flagged_concepts, state.llm_messages, config, resolution_model, parse_rates, user_input, resolutions,
            resolution_model_envelope, failed_parsing_info)
        resolutions.update(new_resolutions)
...
def geo_metadata_column_extraction_approval_node(state: GeoIngestionSubgraphState, *, config: RunnableConfig) -> Command:
    ...
    if exceeded_retries or (not has_formatting_issues and not has_parsing_issues):
        ...
        return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"] = set_step_status(status="completed", step=return_dict["datasets"][accession_code]["steps"]["refine_metadata_schema"])
        ...
        return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"] = set_step_status(status=next_status, step=return_dict["datasets"][accession_code]["steps"]["supplementary_file_check"])
    return Command(update=return_dict)