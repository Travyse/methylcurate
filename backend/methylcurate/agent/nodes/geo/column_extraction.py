...
async def extract_metadata_schema(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("columns:extract_metadata_schema:entry")
    ...
    deps_init: Deps = config["configurable"].get("deps")  # type: ignore[assignment]  # ty: ignore
    if deps_init is not None and deps_init.provenance is not None:
        thread_id = config["configurable"]["thread_id"]
        prov = deps_init.get_provenance(thread_id)
        if prov is not None:
            sampled_gsms = user_input.title if user_input.title else []
            prov.emit_input_loaded(
                artifact_kind="sampled_metadata",
                artifact_path=str(artifact.path),
                artifact_sha256=compute_sha256(str(sorted(sampled_gsms))),
                accession_code=accession_code,
                step="extract_metadata_schema",
                num_samples=len(sampled_gsms),
    ...
    mem_snap("columns:extract_metadata_schema:after_llm")
...
async def check_column_extraction_rule_formatting(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("columns:check_formatting:entry")
    accession_codes = get_accession_codes(state)
    ...
    deps_fmt: Deps = config["configurable"].get("deps")  # type: ignore[assignment]  # ty: ignore
    if deps_fmt is not None and deps_fmt.provenance is not None:
        prov = deps_fmt.get_provenance(config["configurable"]["thread_id"])
        if prov is not None:
            summary = return_dict["datasets"][accession_code].get("metadata_summary", {})
            for concept in get_args(Concept):
                info = summary.get(concept)
                if isinstance(info, dict):
                    prov.emit_validation_completed(
                        concept=concept,
                        parse_rate=info.get("parse_rate"),
                        accession_code=accession_code,
                        step="check_column_extraction_rule_formatting",
                    )
...
async def check_column_extraction_rule_accuracy(state: GeoIngestionSubgraphState, config: RunnableConfig) -> dict[str, Any]:
    mem_snap("columns:check_accuracy:entry")
    accession_codes = get_accession_codes(state)
    running_accession_codes = sorted(
        [accession_code for accession_code in accession_codes if state.datasets[accession_code].steps["refine_metadata_schema"].status == "running"]
    )
    ...
    deps_acc: Deps = config["configurable"].get("deps")  # type: ignore[assignment]  # ty: ignore
    if deps_acc is not None and deps_acc.provenance is not None:
        prov = deps_acc.get_provenance(config["configurable"]["thread_id"])
        if prov is not None:
            summary = return_dict["datasets"][accession_code].get("metadata_summary", {})
            for concept in get_args(Concept):
                info = summary.get(concept)
                if isinstance(info, dict):
                    prov.emit_validation_completed(
                        ...
                        parse_rate=info.get("parse_rate"),
                        accession_code=accession_code,
                        step="check_column_extraction_rule_accuracy",
                    )
...
def geo_metadata_column_extraction_approval_node(state: GeoIngestionSubgraphState, config: RunnableConfig) -> Command:
    ...
    """
    mem_snap("columns:approval_node:entry")
    accession_codes = get_accession_codes(state)
    running_accession_codes = sorted(
        [accession_code for accession_code in accession_codes if state.datasets[accession_code].steps["refine_metadata_schema"].status == "running"]
    )
    ...
    if exceeded_retries or (not has_formatting_issues and not has_parsing_issues):
        if all((r["status"] in ["resolved", "missing"]) and (r["confidence"] > 0.6) for r in resolutions.values()):
            ...
            return_dict["datasets"][accession_code]["metadata_extraction_result"]["artifact"] = artifact_ref.model_dump()
            deps: Deps = config["configurable"].get("deps")  # type: ignore[assignment]  # ty: ignore
            if deps is not None and deps.provenance is not None:
                prov = deps.get_provenance(config["configurable"]["thread_id"])
                if prov is not None:
