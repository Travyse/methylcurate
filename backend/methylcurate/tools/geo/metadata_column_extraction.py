...
from ...utils.error_codes import LLM_UNEXPECTED_ERROR, LLM_VALIDATION_FAILURE
...
def _emit_llm_call_events(provenance, messages, llm_cfg, schema_str, attempt, step, accession_code=None):
    """Emit PromptRendered + LLMRequestStarted before an LLM call."""
...
def _emit_llm_success_events(provenance, result, schema_str, attempt, step, latency_ms, accession_code=None):
    """Emit LLMResponseReceived(True) + LLMTokenUsage after a successful LLM call."""
    if provenance is not None:
...
def _emit_llm_failure_events(provenance, schema_str, attempt, step, latency_ms, accession_code=None):
    """Emit LLMResponseReceived(False) after a failed LLM call."""
    if provenance is not None:
        provenance.emit_llm_response_received(
            latency_ms=latency_ms,
            output_schema=schema_str,
...
async def _invoke_llm_with_retry(
    ...
    Args:
        ...
        accession_code: Optional accession code for provenance logging.
    ...
    _schema = _schema_name(model) if model is not None else "unknown"

    retries = 0
    resolved: Any = None
    while retries < retry_limit:
        _emit_llm_call_events(provenance, messages, llm_cfg, _schema, retries + 1, step, accession_code)
        ...
        try:
            ...
            _emit_llm_success_events(provenance, resolved, _schema, retries + 1, step, _elapsed, accession_code)
            break
        except TimeoutError:
            _elapsed = (time.perf_counter() - _t0) * 1000
            _emit_llm_failure_events(provenance, _schema, retries + 1, step, _elapsed, accession_code)
            if provenance is not None:
...
async def _extract_column_for_concept_age(
    ...
    _schema = _schema_name(FieldResolutionCorrectionDyn)
    _step_name = "_extract_column_for_concept_age"
    _emit_llm_call_events(provenance, prompt_messages, llm_cfg, _schema, 1, _step_name)
    _t0 = time.perf_counter()
    try:
        resolved: Any = await llm.acall_structured(prompt_messages, FieldResolutionCorrectionDyn)
        _elapsed = (time.perf_counter() - _t0) * 1000
        _emit_llm_success_events(provenance, resolved, _schema, 1, _step_name, _elapsed)
        resolutions["age"] = resolved.age
    except ValidationError as e:
        _elapsed = (time.perf_counter() - _t0) * 1000
        _emit_llm_failure_events(provenance, _schema, 1, _step_name, _elapsed)
        if provenance is not None:
            provenance.emit_error_raised(
                error_type="ValidationError",
                error_message=str(e)[:500],
...
async def _extract_column_for_concept_disease_status(
    ...
    _schema = _schema_name(ControlIdentificationModel)
    ...
    _step_name = "_extract_column_for_concept_disease_status"
    while retries < retry_limit:
        _emit_llm_call_events(provenance, call_messages, llm_cfg, _schema, retries + 1, _step_name)
        _t0 = time.perf_counter()
        try:
            resolved: Any = await asyncio.wait_for(
                llm.acall_structured(call_messages, ControlIdentificationModel),
                timeout=CALL_TIMEOUT,
            )
            _elapsed = (time.perf_counter() - _t0) * 1000
            _emit_llm_success_events(provenance, resolved, _schema, retries + 1, _step_name, _elapsed)
            resolutions["disease_status"].extraction.control_value = resolved.control_value
            break
        except TimeoutError:
            _elapsed = (time.perf_counter() - _t0) * 1000
            _emit_llm_failure_events(provenance, _schema, retries + 1, _step_name, _elapsed)
            if provenance is not None:
                provenance.emit_retry_scheduled(
                    error_type="TimeoutError",
                    error_message="Disease status control identification timed out",
                    retry_count=retries + 1,
                    retry_limit=retry_limit,
                    step=_step_name,
                )
...
async def _extract_column_for_concept_misformatted(
    ...
    retry_limit = GLOBAL_RETRY_LIMIT
    retries = 0
    step_name = "_extract_column_for_concept_misformatted"
    while retries < retry_limit:
        _emit_llm_call_events(provenance, new_messages, llm_cfg, _schema, retries + 1, step_name)
        _t0 = time.perf_counter()
        try:
            resolved: Any = await asyncio.wait_for(
            ...
            _elapsed = (time.perf_counter() - _t0) * 1000
            _emit_llm_success_events(provenance, resolved, _schema, retries + 1, step_name, _elapsed)
            for concept in misformatted_concepts:
                new_resolutions[concept] = getattr(resolved, concept)
            break
        except TimeoutError:
            _elapsed = (time.perf_counter() - _t0) * 1000
            _emit_llm_failure_events(provenance, _schema, retries + 1, step_name, _elapsed)
            if provenance is not None:
                provenance.emit_retry_scheduled(
                    error_type="TimeoutError",
                    error_message="Misformatted concept extraction timed out",
                    retry_count=retries + 1,
                    retry_limit=retry_limit,
                    step=step_name,
                )
...
async def _extract_column_for_concept_poor_parsing(
    ...
    retry_limit = GLOBAL_RETRY_LIMIT
    retries = 0
    step_name = "_extract_column_for_concept_poor_parsing"
    while retries < retry_limit:
        _emit_llm_call_events(provenance, new_messages, llm_cfg, _schema, retries + 1, step_name)
        _t0 = time.perf_counter()
        try:
            ...
            if provenance is not None:
                provenance.emit_retry_scheduled(
                    error_type="TimeoutError",
                    error_message="Poor parsing extraction timed out",
                    retry_count=retries + 1,
                    retry_limit=retry_limit,
                    step=step_name,
                )
            ...
            retry_messages = new_messages + [human_message]
            _emit_llm_call_events(provenance, retry_messages, llm_cfg, _schema, retries + 2, step_name)
            _t1 = time.perf_counter()
            resolved: Any = await asyncio.wait_for(
                llm.acall_structured(retry_messages, FieldResolutionCorrectionDyn),
                timeout=CALL_TIMEOUT,
            )
            _elapsed2 = (time.perf_counter() - _t1) * 1000
            _emit_llm_success_events(provenance, resolved, _schema, retries + 2, step_name, _elapsed2)
            for concept in poorly_parsed_concepts:
                new_resolutions[concept] = getattr(resolved, concept)
            break
        except ValidationError as e:
...
async def _extract_all_columns(
    ...
    provenance = deps.get_provenance(config["configurable"]["thread_id"]) if deps else None
    llm_cfg = getattr(llm, "config", None)
    _schema = _schema_name(result_model) if result_model is not None else "unknown"

    retry_limit = GLOBAL_RETRY_LIMIT
    retries = 0
    call_messages = messages
    step_name = "_extract_all_columns"
    while retries < retry_limit:
        print(f"\nAttempt {retries} to extract all columns from metadata...")
        _emit_llm_call_events(provenance, call_messages, llm_cfg, _schema, retries + 1, step_name)
        _t0 = time.perf_counter()
        try:
            ...
            if provenance is not None:
                provenance.emit_retry_scheduled(
                    error_type="TimeoutError",
                    error_message="Extract all columns timed out",
                    retry_count=retries + 1,
                    retry_limit=retry_limit,
                    step=step_name,
                )
...
async def _extract_column_for_concept(messages: list[AnyMessage], config: RunnableConfig) -> FieldResolution:
    ...
    try:
        resolved: Any = FieldResolution.model_validate(await llm.acall_structured(messages, FieldResolution))
    except ValidationError as e:
