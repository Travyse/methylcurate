...
from ...utils.helper import compute_sha256, consolidate_artifacts, read_feather, write_feather
...
async def _process_detection_columns(
    ...
    *,
    ...
    When *file_path* is provided, streams the file in row-chunks to keep
    peak memory bounded.  Otherwise falls back to the existing in-memory
    path using the provided *sample_data*.

    Args:
        artifact: Artifact reference for the sample data file.
        sample_data: DataFrame used for column-scheme detection (light
            sample when streaming, full data otherwise).
        config: LangChain runnable config providing LLM dependencies.
        messages: Prior conversation messages for context.
        file_path: When set, stream from this file instead of using
            *sample_data* for data processing.
        delimiter: Delimiter for the CSV file (required when *file_path*
            is set).
        index_col_name: Name of the column to use as the probe-ID index
    ...
    mem_snap("_process_detection_columns:after_column_scheme")
    ...
    stream = file_path is not None

    if stream:
        assert delimiter is not None
        return await _stream_process_detection_columns(artifact, file_path, delimiter, index_col_name, beta_cols, det_cols, det_pat, beta_pat)

    sample_data = sample_data.apply(pd.to_numeric, errors="coerce")
    mem_snap("_process_detection_columns:after_numeric_conversion")
...
async def _stream_process_detection_columns(
    artifact: ArtifactRef,
    file_path: str,
    delimiter: str,
    ...
    with tempfile.TemporaryDirectory(dir="/app/outputs/_tmp") as tmpdir:
        reader = pd.read_csv(
            ...
            sep=delimiter,
            comment="#",
            chunksize=DETECTION_CHUNK_SIZE,
        )
        for chunk_idx, chunk in enumerate(reader):
            if index_col_name is None and chunk_idx == 0:
                for col in chunk.columns[:5]:
                    matching_count = sum(1 for x in chunk[col].dropna()[:100] if isinstance(x, str) and x.lower().startswith("cg"))
                    if matching_count / min(100, len(chunk)) > 0.8:
                        chunk = chunk.set_index(col)
                        chunk = chunk[~chunk.index.isna()]
                        break

            chunk.columns = [x.lower().strip() for x in chunk.columns]
            chunk = chunk.reindex(columns=stream_cols)
            ...
            del chunk, chunk_beta, filtered, chunk_df
            if det_pat and det_cols:
                del chunk_det
                if "mask" in dir():
                    del mask
        ...
        dfs = [read_feather(p) for p in chunk_paths]
        methylation_df = pd.concat(dfs, axis=1, copy=False)
        del dfs, chunk_paths
        gc.collect()
...
async def _process_detection_columns_alt(
    ...
    for pair in paired_columns:
        ...
        data = data[data["Detection_Pval"] < 0.05]
...
def _read_sample_data_light(file_path: str) -> tuple[pd.DataFrame, str, str | None]:
    """Read a lightweight sample of a supplementary file for column layout detection.

    Reads only enough rows to determine column structure, avoiding
    allocating the full file. Returns the sample DataFrame, detected
    delimiter, and index column name (if one was set).

    Args:
        file_path: Path to the data file (may be .gz).

    Returns:
        A tuple of (sample_df, delimiter, index_col_name_or_None).
    """
    ...
    n_header_cols = len(first_line.strip().split(delimiter))
    nrows = 2000 if n_header_cols < 5000 else 20

    sample_data = pd.read_csv(file_path, sep=delimiter, comment="#", nrows=nrows, index_col=False)
    index_col_name = None
    ...
    return sample_data, delimiter, index_col_name
...
async def _get_column_scheme(
    ...
    schema_str = _schema_name(ForcedSampleDataResolution)
    ...
    retries = 0
    step_name = "_get_column_scheme"
    resolved: Any = None
    while retries < retry_limit:
        prompt_sha = _message_sha256(messages)  # ty: ignore
        msg_count = _message_count(messages)  # ty: ignore
        if provenance is not None:
            provenance.emit_prompt_rendered(
                template_name="_get_column_scheme",
                template_path="",
                rendered_sha256=prompt_sha,
                rendered_preview=_render_preview(messages),  # ty: ignore
                accession_code=getattr(artifact, "accession_code", None),
                step=step_name,
            )
        ...
        try:
            resolved = await asyncio.wait_for(
                llm.acall_structured(messages, ForcedSampleDataResolution),
            ...
            if provenance is not None:
                ...
                provenance.emit_retry_scheduled(
                    ...
                    error_message="Column scheme resolution timed out",
    ...
    if resolved is None:
        raise ValueError(f"Column scheme resolution failed after {retry_limit} retries for artifact {artifact.path}")
    ...
    if prev_beta_pattern or prev_detection_pattern:
        if _check_pattern_performance_change(
            ...
            prev_detection_pattern,
            resolved.beta_column.pattern,  # type: ignore
    ...
    if count < 3:
        ...
        beta_pattern = re.compile(resolved.beta_column.pattern, re.IGNORECASE)  # type: ignore
...
async def format_individual_methylation_data(
    ...
    mem_snap(f"format_individual:before_read {accession_code}")
    sample_data, delimiter, index_col_name = _read_sample_data_light(artifact.path)
    has_detection = _check_for_detection_columns(sample_data)
    mem_snap(f"format_individual:after_read {accession_code}")
    if has_detection:
        df, _ = await _process_detection_columns(
            artifact,
            sample_data,
            config,
            messages,
...
async def format_methylation_data(
    ...
    formatted_data = pd.concat(formatted_datasets, axis=0).astype(np.float32)
...
def lexical_score_matrix(A: list[str], B: list[str], workers: int = 0, no_count_penalty: bool = False) -> np.ndarray:
...
def _extract_best_subject_id_fields(user_input: Any, target_values: list[str]) -> tuple[str | None, str | None]:
    ...
    Args:
        ...
        target_values: Sorted list of sample identifiers from the
            methylation data index.
...
def _create_subject_id_mapping(
    accession_code: str, user_input: pd.DataFrame, metadata_dict: dict[str, Any], target_values: list[str]
) -> pd.DataFrame:
    ...
    def _get_subject_column_values(source_values: list[str], target_values: list[str]) -> Any:
        ...
        print("Ordered target values has been assigned.")
        return ordered_target_values

    field_name, key_name = _extract_best_subject_id_fields(user_input, target_values)

    gsm_candidates: dict[str, list[str]] = {}
    all_candidates: set[str] = set()
    for gsm_name, gsm in metadata_dict["sample_metadata"].items():
        candidates: list[str] = []
        if field_name in gsm:
            if field_name == "characteristics_ch1":
                for item in gsm[field_name]:
                    k, k_value = item.split(":")
                    if k.strip() == key_name:
                        candidates.append(k_value)
            else:
                for item in gsm[field_name]:
                    candidates.append(item)
        candidates = list(sorted(set(candidates)))
        gsm_candidates[gsm_name] = candidates
        all_candidates.update(candidates)

    unique_candidates = sorted(all_candidates)
    score_matrix = lexical_score_matrix(unique_candidates, target_values, no_count_penalty=True)

    candidate_best: dict[str, tuple[float, str]] = {}
    best_j = np.argmax(score_matrix, axis=1)
    for i, j in enumerate(best_j):
        candidate_best[unique_candidates[i]] = (score_matrix[i, j], target_values[j])

    rows = []
    for gsm_name, candidates in gsm_candidates.items():
        subject = None
        if candidates:
            subject = max(candidates, key=lambda c: candidate_best[c][0])
            score, sample = candidate_best[subject]
            print(f"\nExtracted subject {subject} for value {sample} in field {field_name} and key {key_name} with score {score}")
        rows.append({"Sample": gsm_name, "Subject": subject})

    df = pd.DataFrame(rows)
    print("\nConstructed subject mapping DataFrame:\n")
    df["Beta_Subjects"] = _get_subject_column_values(df["Subject"].tolist(), target_values)
    print("Beta_Subjects column added to mapping DataFrame:\n")
    return df
