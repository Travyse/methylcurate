...
def _impute_per_split(
    ...
    imputer_input: ImputerModelInput,
    split_strategy: ImputationInput,
) -> pd.DataFrame:
    ...
    imputers = {split_value: ESTIMATOR_REGISTRY[concept](**imputer_kwargs) for split_value in unique_split_values}
    imputed_target_dfs = []
    for split_value in unique_split_values:
        ref_indices = reference_metadata_df[reference_metadata_df[split_strategy.stratify_by] == split_value].index
        tgt_indices = target_metadata_df[target_metadata_df[split_strategy.stratify_by] == split_value].index
