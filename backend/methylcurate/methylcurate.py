...
def parse_args() -> CLIConfig:
    ...
    parser.add_argument(
        "--llm-config",
        default=None,
        help="Path to a YAML LLM config file. Defaults to LLM_CONFIG_PATH env var.",
    )
...
async def chat_loop(cfg: CLIConfig):
    ...
    runner, deps = build_services_with_checkpointer(checkpointer, config_path=cfg.llm_config)
