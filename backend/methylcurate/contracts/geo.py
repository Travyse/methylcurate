...
class GEODownloadResult(BaseModel):
    ...
    """
    accession: NonEmptyStr = Field(..., description="GSE accession, e.g. GSE40279")
    artifact: Optional[ArtifactRef] = Field(..., description="Reference to the downloaded artifact, if download was successful")
...
class GEOMetadataExtractionInput(BaseModel):
    ...
    platform_id: Optional[List[List[NonEmptyStr]]] = Field(None, description="List of example platform IDs from GEO metadata")
...
class ExtractionRuleBase(BaseModel):
    ...
    group_index: int = Field(..., le=1, description=(
...
class DefaultValue(BaseModel):
    ...
    value: NonEmptyStr = Field(..., description="The default value to use for this concept when no extractable evidence is found in the dataset but there is evidence in the dataset metadata like title or summary. This should be a single, specific value")
...
class CharacteristicsExtractionRule(ExtractionRuleBase):
    ...
    pattern: NonEmptyStr = Field(..., description="A regular expression pattern to apply to the specified key_name value in characteristics_ch1. For example, if key_name is 'age' and characteristics_ch1 includes the following entry: {'age': '45 years'}, then this pattern would be applied to '45 years' and should extract the concept value of '45'.")
    key_name: NonEmptyStr = Field(..., description="The key inside the target key:value string in characteristics_ch1, e.g. 'age' in 'age: 45 years'")
    control_value: Optional[NonEmptyStr] = Field(None, description="This only set for extraction rules for disease_status. This identifies how the dataset encodes the control samples, e.g. 'healthy' or 'control'.")
...
class OtherExtractionRule(ExtractionRuleBase):
    ...
    pattern: NonEmptyStr = Field(..., description="A regular expression pattern to apply to the specified field. For characteristics_ch1 extractions, this pattern will be applied to each key:value pair in the list. This regex pattern should extract the concept value.")
...
class MissingResolution(FieldResolutionBase):
    ...
    candidate_key_names: List[str] = Field(
        ..., min_length=1, description="Key names in characteristics_ch1 checked and found no extractable evidence for this concept. Must include every possible key name."
    )
    absence_evidence: List[NonEmptyStr] = Field(
        ..., min_length=1, description="Must be verbatim substrings copied from the input values, not analyst commentary. Never write 'no field contains…'"
    )
...
class GEOMetadataExtractionResult(BaseModel):
    ...
    """
    model_config = ConfigDict(extra="forbid")
    artifact: Optional[ArtifactRef] = Field(..., description="Reference to the saved json file containing the metadata schema")
...
def build_dynamic_result_model(allowed_keys: Tuple[str, ...]):
    ...
    CharacteristicsExtractionRuleDyn = create_model(
        ...
        field_name=(Literal["characteristics_ch1"], Field(..., description="The name of the field in the input data that contains the concept.")),
        pattern=(NonEmptyStr, Field(..., description="A regular expression pattern to apply to the specified key_name value in characteristics_ch1. For example, if key_name is 'age' and characteristics_ch1 includes the following entry: {'age': '45 years'}, then this pattern would be applied to '45 years' and should extract the concept value of '45'.")),
        key_name=(KeyName, Field(..., description="This represents the key name of one of the key: value pairs in characteristics_ch1 that semantically encodes the concept. This is not necessarily an exact match of the concept. For example, if the concept is age and the characteristics_ch1 column includes 'age (years): 45', then key_name would be 'age'")),
        control_value=(Optional[NonEmptyStr], Field(None, description="This only set for extraction rules for disease_status. This identifies how the dataset encodes the control samples, e.g. 'healthy' or 'control'.")),
    )
    ...
    MissingResolutionDyn = create_model(
        ...
        candidate_field_names=(List[Literal["title","source_name_ch1","description","characteristics_ch1","relation","platform_id"]], Field(..., min_length=6, description="Fields checked and found no extractable evidence for this concept.")),
        candidate_key_names=(List[KeyName], Field(..., min_length=len(get_args(KeyName)), description="Key names in characteristics_ch1 checked and found no extractable evidence for this concept. Must include every possible key name.")),
        absence_evidence=(List[NonEmptyStr], Field(..., min_length=1, description="Must be verbatim substrings copied from the input values, not analyst commentary. Never write 'no field contains…'")),
    )
...
def build_dynamic_resolution_correction_model(allowed_concepts: Tuple[str, ...], field_resolution_dyn: Any):
    ...
    model_fields = {
        c: (field_resolution_dyn, Field(..., description=f"Resolution details for {c} concept")) for c in allowed_concepts
    }
...
def build_dynamic_control_identification_model(allowed_values: Tuple[str, ...]):
    ...
    ControlIdentificationModel = create_model(
        ...
        __base__=BaseModel,
        __config__=ConfigDict(extra="forbid"),
        control_value=(Optional[Literal[allowed_values]], Field(..., description="The value in the dataset that indicates a control sample, e.g. 'healthy' or 'control'"))
    )
...
def build_dynamic_constrained_resolution_model(allowed_keys: Tuple[str, ...], target_concept: str):
    ...
    OtherExtractionRuleDyn = create_model(
        ...
        __base__=BaseModel,
        field_name=(OtherKeys, Field(..., description="The name of the field in the input data that contains the concept.")))
...
class MissingColumnExtraction(BaseModel):
    ...
    absence_evidence: List[NonEmptyStr] = Field(..., min_length=1, description="Must be verbatim substrings copied from the input values, not analyst commentary. Never write 'no column contains…'")
...
class SampleDataResolution(BaseModel):
    ...
    detection_column: ColumnExtraction = Field(..., description="Extraction details for the detection p-value column, if applicable. This column contains p-values for each of the CpG probes.")
...
class ForcedSampleDataResolution(BaseModel):
    ...
    detection_column: MissingOrResolved = Field(..., description="Extraction details for the detection p-value column, if applicable. This column contains p-values for each of the CpG probes.")
...
class LexFeat(BaseModel):
    ...
    norm: str = Field(..., description="Normalized version of the string for easier comparison, e.g. lowercased, split camelCase, and stripped of special characters")
...
class GEOSampleLevelMetadata(BaseModel):
    ...
    subject_id: Optional[NonEmptyStr] = Field(None, description="The subject ID associated with this sample, if available")
    ...
    cell_type: Optional[NonEmptyStr] = Field(None, description="The cell type associated with this sample, if available")
    status: Optional[NonEmptyStr] = Field(None, description="The disease status associated with this sample, if available")
    sex: Optional[NonEmptyStr] = Field(None, description="The sex of the subject associated with this sample, if available")
    platform: Optional[NonEmptyStr] = Field(None, description="The platform ID associated with this sample, if available")
