...
class QcConfig(BaseModel):
    dnam_cutoff: float | None = Field(None, ge=0.0, le=1.0, description="Minimum allowed DNAm value per sample")
    sample_level_missing_cutoff: float | None = Field(None, ge=0.0, le=1.0, description="Max allowed fraction of missing values per sample")
    cpg_level_missing_cutoff: float | None = Field(None, ge=0.0, le=1.0, description="Max allowed fraction of missing values per CpG site")
    correlation_cutoff: float | None = Field(None, ge=0.0, le=1.0, description="Min allowed mean inter-array correlation per sample")
    ...
    @classmethod
    def _interpolate_value(cls, value):
    ...
    @classmethod
    def from_yaml(cls, path: str | Path) -> "QcConfig | None":
        resolved = os.path.expandvars(str(path))
        ...
        try:
            ...
            raise ImportError("pyyaml is required to load QC config from a YAML file. Install it with: pip install pyyaml>=6.0") from err
        ...
        if not isinstance(raw, dict):
            raise ValueError(f"QC config file must contain a YAML mapping, got: {type(raw).__name__}")

        known_fields = {"dnam_cutoff", "sample_level_missing_cutoff", "cpg_level_missing_cutoff", "correlation_cutoff"}
