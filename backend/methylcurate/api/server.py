...
async def stream_thread(thread_id: str, req: StreamRequest, request: Request):
    ...
    async def run_bg():
        ...
        try:
            ...
            logger.exception("Background runner crashed for thread %s", thread_id)
            try:
                await session.queue.put(type("Ev", (), {"type": "final", "payload": {"message": f"Runner crashed: {e}. See server logs."}}))
            except Exception:
                pass
