...
def generate_geo_metadata_extraction_examples(n_samples=10, concept: str = "age", is_missing=False):
    ...
    example_input = json.dumps({
        ...
        "description": [[]],
        "characteristics_ch1": [{k: v for k, v in _generate_random_characteristics_ch1(concept, is_missing).items() if v is not None} for _ in range(n_samples)],
        "relation": None,
    ...
    if is_missing or concept == "condition":
        resolution_dict.update({
            "status": "missing",
            "notes": [f"{concept} is not present in the sample metadata."]
        })
    else:
        resolution_dict.update({
            "extraction": {
...
def generate_general_geo_metadata_extraction_examples(n_samples=10, is_missing=True):
    ...
    example_input = json.dumps({
        ...
        "source_name_ch1": [[random.choice(["Prefrontal Cortex", "Hippocampus", "Cerebellum"])] for _ in range(n_samples)],
        "description": [[]],
        "characteristics_ch1": [{k: v for k, v in _generate_random_characteristics_ch1(is_missing).items() if v is not None} for _ in range(n_samples)],
        "relation": None,
        "platform_id": [["GPL12345"] for _ in range(n_samples)]
    }, indent=2, ensure_ascii=False)
    ...
    example_response = json.dumps({
        ...
        "cell_type": {
            ...
            "candidate_field_names": [
                "title", "source_name_ch1", "description", "characteristics_ch1", "relation", "platform_id"
            ],
            "candidate_key_names": [
                "tissue", "brain bank", "post-mortem delay", "braak stage", "donor_id", "age (years)", "Sex", "ad diagnosis"
            ],
            "confidence": 0.95,
...
def generate_metadata_harmonization_examples() -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    example_input = {
        "concept": "tissue",
        "unique_values": [
            "pfc", "prefrontal cortex", "brain: prefrontal cortex", "pre-frontal cortex", "prefrontalcortex"]
    }
...
def generate_column_interpretation_examples(alt = False) -> tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    """
    example_input = {
        "columns": ["sample_1_beta", "sample_1_Detection_pval", "sample_2_beta", "sample_2_Detection_pval", "sample_3_beta", "sample_3_Detection_pval", "sample_4_beta", "sample_4_Detection_pval", "sample_5_beta", "sample_5_Detection_pval"],
        "rows": [
    ...
    example_agent_response = {
        "beta_column": {
            ...
            "column_evidence": [df.columns.tolist()[i] for i in range(len(df.columns)) if re.search("^[a-zA-Z0-9]+_[a-zA-Z0-9]+_beta$" if not alt else "^\d+-[a-zA-Z0-9]+_[a-zA-Z0-9]+$", df.columns[i], re.IGNORECASE)],
            "evidence": ["Identified beta value columns based on the '_beta' suffix in column names." if not alt else "Identified beta-like values in these columns which are bound between 0 and 1, and are not detection columns."]
        },
        "detection_column": {
            "status": "resolved",
            "pattern": "^[a-zA-Z0-9]+_[a-zA-Z0-9]+_Detection_pval$" if not alt else "^\d+-[a-zA-Z0-9]+_[a-zA-Z0-9]+_Detection_pval$",
            "column_evidence": [df.columns.tolist()[i] for i in range(len(df.columns)) if re.search("^[a-zA-Z0-9]+_[a-zA-Z0-9]+_Detection_pval$" if not alt else "^\d+-[a-zA-Z0-9]+_[a-zA-Z0-9]+_Detection_pval$", df.columns[i], re.IGNORECASE)],
            "evidence": ["Identified detection p-value columns based on the '_Detection_pval' suffix in column names." if not alt else "Identified _Detection_pval prefix in these columns"]
        },
    }
...
def generate_column_interpretation_examples_no_detection() -> tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    example_input = {
        "rows": [
            [random.uniform(0, 1) for _ in range(5)] for _ in range(5)
        ],
        "index": ["cg00000029", "cg00000108", "cg00000109", "cg00000165", "cg00000236"]
    }
    cols = [ f"{random.randint(1, 10)}-{uuid.uuid4().hex[:4]}_{uuid.uuid4().hex[:8]}" for _ in range(5)]
    ...
    example_agent_response = {
        ...
        "detection_column": {
            ...
            "absence_evidence": ["No columns contain evidence of being detection p-value columns such as having 'detect' or 'pval' in the column name."]
        },
    }
...
def _high_level_ontology_labels(ontology:str) -> tuple[list[str], Any, Any]:
    ...
    """
    if ontology == "mondo":
        labels =  ["Alzheimer disease", "Parkinson disease", "Lewy body dementia", "schizophrenia", "HIV infectious disease", "diabetes mellitus"]
...
def _field_names(ontology:str) -> Dict[str, str]:
    ...
    if ontology == "mondo":
        return {
            "metadata_field_name": "characteristics_ch1",
            "metadata_key_name": "diagnosis"
        }
    elif ontology == "uberon": 
        return {
            "metadata_field_name": "characteristics_ch1",
            "metadata_key_name": "tissue"
        }
    elif ontology == "cl":
        return {
            "metadata_field_name": "characteristics_ch1",
            "metadata_key_name": "cell_type"
        }
    elif ontology == "pato":
        return {
            "metadata_field_name": "characteristics_ch1",
            "metadata_key_name": "sex"
        }
    else:
        raise ValueError(f"Unsupported ontology: {ontology}")
    ...
    if ontology == "mondo":
        ...
        target_label_dict = {
            "dementia with lewy bodies": [
                "Lewy body dementia", "progressive dementia with neuroserpin inclusion bodies", "familial encephalopathy with neuroserpin inclusion bodies", "Pick disease", "early-onset Lafora body disease"
            ],
            "alzheimer disease": [
                "Alzheimer disease", "Alzheimer disease, degu", "Alzheimer disease, dog", "Alzheimer disease, sheep", "Alzheimer disease, domestic cat"
            ],
            "mild cognitive impairment": [
                "mild cognitive impairment", "adult onset demyelinating leukodystrophy", "spinocerebellar ataxia 19/22", "dystonia 22, adult-onset", "Smith-Magenis syndrome"
            ]
        }
        ...
        target_label_dict = {
            "anterior cingulate cortex": [
                "anterior cingulate cortex", "rostral anterior cingulate cortex", "caudal anterior cingulate cortex", "anterior cingulate gyrus", "cingulate cortex"
            ],
            "substantia nigra": [
                "substantia nigra", "substantia nigra pars reticulata", "substantia nigra pars compacta", "substantia nigra pars lateralis", "midbrain tegmentum"
            ]
        }
        target_labels = sorted(set([label for sublist in target_label_dict.values() for label in sublist]))
        _, LabelMappingSetDyn = create_ontology_mapping_model(
        ...
        target_label_dict = {
            "microglia": [
                "microglial cell", "microglial cell (Mmus)", "immature microglial cell", "mature microglial cell", "astrocyte"
            ],
            "astrocytes": [
                "astrocytes", "astrocyte-restricted precursor", "astrocyte of the forebrain", "astrocyte of the cerebellum", "astrocyte of the cerebrum (Mmus)"
            ],
            "neurons": [
                "neuron", "neuronal receptor cell", "neuron associated cell", "neuron of the forebrain", "neuronal-restricted precursor"
            ]
        }
        target_labels = sorted(set([label for sublist in target_label_dict.values() for label in sublist]))
        _, LabelMappingSetDyn = create_ontology_mapping_model(
    ...
    if ontology == "mondo":
        ...
        target_label_dict = {
            "neurodegenerative disease": [
                'neurodegenerative disease', 'neurodegenerative disease, non-human animal', 'Huntington disease-like 1', 'inherited neurodegenerative disorder', 'neurodegenerative vacuolar storage disease, dog'
            ],
            "psychiatric disorder": [
                'psychiatric disorder', 'psychiatric disorder, non-human animal', 'myoclonic epilepsy, congenital deafness, macular dystrophy, and psychiatric disorders', 'psychogenic movement disorders', 'Huntington disease-like 1'
            ],
            "infectious disease": [
                'infectious disease', 'infectious disease characteristic', 'infectious disease with sepsis', 'infectious disease, non-human animal', 'HHV-7 infectious disease'
            ]
        }
        target_labels = sorted(set([label for sublist in target_label_dict.values() for label in sublist]))
        _, LabelMappingSetDyn = create_ontology_mapping_model(
        ...
        target_label_dict = {
            "brain": [
                'brain', 'forebrain', 'midbrain', 'hindbrain', 'brainstem'
            ],
            "sigmoid colon": [
                'sigmoid colon', 'colon', 'rectum', 'proximal-distal subdivision of colon', 'hindgut'
            ],
            "artery": [
                'artery', 'endothelium of artery', 'pharyngeal arch artery', 'maxillary artery', 'ovarian artery'
            ]
        }
        target_labels = sorted(set([label for sublist in target_label_dict.values() for label in sublist]))
        _, LabelMappingSetDyn = create_ontology_mapping_model(
