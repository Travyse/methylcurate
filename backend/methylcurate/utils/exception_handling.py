...
def classify_geo_error(e: Exception) -> Tuple[str, str, str]:
    ...
    if isinstance(e, (
        ...
        return (
            "network_error",
            "Network connection failed. Please check your internet or firewall.",
            str(e)
        )
    ...
    if isinstance(e, (
        ...
        return (
            "ftp_error",
            "Unable to reach GEO via FTP. Check your network or try again later.",
            str(e)
        )
    ...
    if isinstance(e, (
        ...
        if hasattr(e, "errno"):
            if e.errno == 28:  # No space left on device
                return (
                    "disk_full",
                    "There is not enough disk space to download this dataset.",
                    "Disk full (errno 28)"
                )

        return (
            "filesystem_error",
            "There was a problem writing files to disk. Check permissions and space.",
            str(e)
        )
    ...
    if isinstance(e, RuntimeError):
        return (
            "geo_parse_error",
            "This GEO record could not be parsed correctly.",
            str(e)
        )
    ...
    return (
        "unknown_error",
        "An unexpected error occurred while downloading the dataset.",
        repr(e)
    )