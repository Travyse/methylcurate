...
function toThreadMessageLike(m: LCMsg): ThreadMessageLike {
  // 1) If it's a ToolMessage with a table payload, emit a tool-call part
  if (m.type === "tool" && m.artifact && m.artifact.columns && (m.artifact.data || m.artifact.rows)) {
    const toolName =
      m.name ??                          // if your backend provides name
      (m.artifact.id === "geo-dataset-summary" ? "geoDatasetSummary" : "unknownTool");

    const result = {
      ...m.artifact,
      // normalize if your DataTable expects `data` not `rows`
    ...
    return {
      ...
      content: [
        ...
        // THIS is what triggers toolkit rendering:
        {
          type: "tool-call",
          toolCallId: m.tool_call_id ?? `synthetic-${m.artifact.id ?? crypto.randomUUID()}`,
          toolName,
          args: {},               // optional
          result,                 // <-- DataTable payload lands here
  ...
  // 2) fallback: your existing text mapping
