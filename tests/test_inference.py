...
class TestMutableDefaultArguments:
    ...
    def test_mutable_default_does_not_persist_state(self):
        ...
        extraction_protocol = {
            "disease_status": {"extraction": {"control_value": "Control"}}
        }
    ...
    def test_shuffled_labels_uses_n_bootstraps_not_hardcoded(self):
        ...
        for func in (bootstrap_welch_one_sided_aac_gt_hc, bootstrap_aa1_test):
            ...
            assert "range(n_bootstraps)" in source, (
                f"{func.__name__} should use n_bootstraps parameter, "
                "not hardcoded 1000"
            )
            assert "range(1000)" not in source, (
                f"{func.__name__} contains hardcoded range(1000); "
                "should use range(n_bootstraps)"
            )
...
class TestBootstrapRespectsNBootstrapsRuntime:
    ...
    def test_n_bootstraps_controls_shuffled_labels_count(self, mock_test, mock_parallel):
        ...
        result = bootstrap_welch_one_sided_aac_gt_hc(
            df, extraction_protocol, clocks=["horvath"], n_bootstraps=75
        )
